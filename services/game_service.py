...
from schemas.game import GameOut, GameDetail
...
class GameService:
    ...
    async def list_games(self, category: Optional[str] = None, page: int = 1, per_page: int = 20):
        ...
        game_list = [
            GameOut(
                id=g.id, name=g.name, slug=g.slug, thumbnail=g.thumbnail.replace('/games/', '/static/games/', 1),
    ...
    async def get_by_slug(self, slug: str) -> Optional[GameDetail]:
        ...
        if not game:
            return None
        return GameDetail(
            id=game.id, name=game.name, slug=game.slug, thumbnail=game.thumbnail.replace('/games/', '/static/games/', 1),
